...
- (void)close {
    ...
    @try {
        ...
        [closeEventString appendFormat:@",\"%@\":%lu", PARAM_CLIENT_TIME, (long)[self getTimestamp]];
...
- (void)tagEvent:(NSString *)event attributes:(NSDictionary *)attributes reportAttributes:(NSDictionary *)reportAttributes {
    @try {
        ...
        [eventString appendFormat:@",\"%@\":%lu", PARAM_CLIENT_TIME, (long)[self getTimestamp]];
...
- (BOOL)createOptEvent:(BOOL)optState {
    ...
    [optEventString appendFormat:@",\"%@\":%lu", PARAM_CLIENT_TIME, (long)[self getTimestamp]];
	[optEventString appendString:@"}\n"];
...
- (NSString *)getGlobalDeviceId {
    ...
    if (theUUID) {
        systemId = NSMakeCollectable(CFUUIDCreateString(kCFAllocatorDefault, theUUID));
        CFRelease(theUUID);
    }
	if (systemId == nil) {
		return nil;
	}
	return [systemId autorelease];
}
